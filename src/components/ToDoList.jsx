  ...
  return(
    <div>
      <div ref={note} onClick={expandCenter} className="note" style={isClicked ? customNoteStyles : null}>
        ...
        {props.content.map(toDoItem => <ToDoItem key={props.noteId} noteId={props.noteId} toDoItemId={toDoItem.key} toDoItem={toDoItem.content} deleteToDoItem={props.deleteToDoItem} changeToDoItem={props.changeToDoItem} isClicked={isClicked}/>)}

        <div className="todo-wrapper">
          {isClicked ? <AddIcon className="add-icon"/> : null}
          {isClicked ? <TextareaAutosize className="todo-input" name="listItem" placeholder="List Item" minRows={1} onChange={changeContent}
